...
class LabelsControllerTest {
    ...
    void testIndex() throws Exception {
        ...
        var list = om.readValue(data, new TypeReference<List<LabelResponseDTO>>(){});
        var stored = labelRepository.findAll();
        var actualIds = list.stream().map(LabelResponseDTO::getId).toList();
        var expectedIds = stored.stream().map(Label::getId).toList();
